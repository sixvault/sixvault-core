    ...
    try {
        ...
        if (!nim_nip || !type || !prodi || !rsaPublicKey || !nama) {
            return res.status(400).json({
                status: 'error',
                message: 'Parameter "nim_nip", "type", "prodi", "rsaPublicKey", and "nama" required',
                data: {}
            });
        }
        ...
        if (!["mahasiswa", "dosen_wali", "kaprodi"].includes(type)) {
            return res.status(400).json({
                status: 'error',
                message: 'Parameter "type" must be "mahasiswa", "dosen_wali", or "kaprodi"',
                data: {}
            });
        }

        if (!["teknik_informatika", "sistem_dan_teknologi_informasi"].includes(prodi)) {
            return res.status(400).json({
                status: 'error',
                message: 'Parameter "prodi" must be "teknik_informatika" or "sistem_dan_teknologi_informasi"',
                data: {}
            });
        }
        ...
        const aes = new AES();
        const encryptedAccessToken = aes.encrypt(signAccessToken(userTokenSign), tokenKey);
        const encryptedRefreshToken = aes.encrypt(signRefreshToken(userTokenSign), tokenKey);
...
const login = async (req, res) => {
    try {
        ...
        if (!nim_nip || !rsaPublicKey) {
            return res.status(400).json({
                status: 'error',
                message: 'Parameter "nim_nip" and "password" required',
                data: {}
            });
        }
        ...
        if (!user) {
            return res.status(400).json({
                status: 'error',
                message: process.env.DEBUG ? "User not Found" : "Invalid Credentials",
                data: {}
            });
        }
